...
class HIEST(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.transformer_adj_mx = self.adj_mx
    ...
    def forward(self, batch):
        ...
        for b in range(self.layers):#2
            for i in range(self.blocks):#4
                #            |----------------------------------------|     *residual*
                #            |                                        |
                #            |    |-- conv -- tanh --|                |
                # -> dilate -|----|                  * ----|-- 1x1 -- + -->	*input*
                ...
                # dilated convolution
                filter = self.filter_convs[4*b+i](residual)
                # (batch_size, dilation_channels, num_nodes, receptive_field-kernel_size+1)
                filter = torch.tanh(filter)

                gate = self.gate_convs[4*b+i](residual)
                # (batch_size, dilation_channels, num_nodes, receptive_field-kernel_size+1)
                ...
                '''
                x, xc, xs = self.gconv[4*b+i](x, self.supports, self.supports_r, Mrg)
                x = x + residual[:, :, :, -x.size(3):]
                x = self.bn[4*b+i](x)
                xc = self.bnc[4*b+i](xc)
                xs = self.bns[4*b+i](xs)
                '''

            x, xc, xs = self.gconv[b](x, self.supports, self.supports_r, Mrg)
            x = x + residual[:, :, :, -x.size(3):]
            x = self.bn[b](x)
            xc = self.bnc[b](xc)
            xs = self.bns[b](xs)
